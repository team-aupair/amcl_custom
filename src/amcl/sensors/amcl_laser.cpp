...
bool AMCLLaser::UpdateSensor(pf_t *pf, AMCLSensorData *data, double x, double y, double r)
{
  if (this->max_beams < 2)
    return false;
...
double AMCLLaser::LikelihoodFieldModel(AMCLLaserData *data, pf_sample_set_t* set, double x, double y, double r, int use_orb)
{
	AMCLLaser *self;
	int i, j, step;
	double z, pz;
	...
	for (j = 0; j < set->sample_count; j++)
	{
		...
		if (use_orb) {
			...
			if (dist > 1.0) p *= 0.2;
			else if (dist > 0.1) p *= 1 - (0.8 * (dist - 0.1) / 0.9);
