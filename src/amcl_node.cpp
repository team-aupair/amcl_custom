        ...
        first_reconfigure_call_(true)
{
  ...
  private_nh_.param("use_orb_slam", use_orb_slam_, true);
  private_nh_.param("orb_frame_id", orb_frame_id_, std::string("orb_base_link"));
...
/*ORB
void AmclNode::setTfCallback()
{
	tf::StampedTransform transform;
	try {
		tf_.lookupTransform("/map", "/orb_base_link", ros::Time(0), transform);
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  if(lasers_update_[laser_index])
  {
	...
	else lasers_[laser_index]->UpdateSensor(pf_, (AMCLSensorData*)&ldata);
